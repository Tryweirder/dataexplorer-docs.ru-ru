...
* `[clustername].[geo-region].kusto.windows.net`ядре `ingest-[clustername].[geo-region].kusto.windows.net` (Управление данными) сопоставляется с общедоступным IP-адресом для каждой службы. 

* `private-[clustername].[geo-region].kusto.windows.net`ядре `private-ingest-[clustername].[geo-region].kusto.windows.net` (Управление данными) сопоставляется с частным IP-адресом для каждой службы.
...
| Порты, используемые для `sql\_request` подключаемых `http\_request` модулей и  | Подсеть ADX  | Интернет: настраиваемый  | TCP  |
...
| Восточный Китай 2 | 139.217.236.210 |
| Северный Китай 2 | 40.73.6.21 |
...
| Центральная Восточная | 52.182.33.66 |
| Восточная Восток | 52.181.33.69 |
...
| Центральная Восточная | 52.238.116.34 |
| Восточная Восток | 52.238.116.34 |
...
| Центрально-южная часть США | 13.84.173.99 |
