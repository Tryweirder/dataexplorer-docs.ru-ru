...
[**mv-expand**](kusto/query/mvexpandoperator.md): расширяет многостоимостью коллекцию (ы) из динамического столбца, чтобы каждое значение в коллекции получило отдельный ряд. Все остальные столбцы в развернутой строке дублируются. Действие этого оператора противоположно действию оператора makelist.
