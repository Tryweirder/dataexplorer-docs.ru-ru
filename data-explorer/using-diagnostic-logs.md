...
> [!NOTE]
> При отправке журналов в log Analytics `SucceededIngestion` журналы,, `FailedIngestion` `Command` и `Query` будут храниться в log Analytics таблицах с именами: `SucceededIngestion` ,, `FailedIngestion` `ADXCommand` , `ADXQuery` соответственно.
