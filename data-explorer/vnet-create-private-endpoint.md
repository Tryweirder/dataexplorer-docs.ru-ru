...
ms.locfileid: "90067578"
---
# <a name="create-a-private-endpoint-in-your-azure-data-explorer-cluster-in-your-virtual-network"></a>Создание частной конечной точки в кластере Azure обозреватель данных в виртуальной сети
...
Чтобы настроить [службу частной связи](https://docs.microsoft.com/azure/private-link/private-link-service-overview), используйте закрытую конечную точку с IP-адресом из адресного пространства виртуальной сети Azure. [Частная конечная точка Azure](https://docs.microsoft.com/azure/private-link/private-endpoint-overview) использует частный IP-адрес из виртуальной сети для безопасного подключения к Azure обозреватель данных в частном порядке. Кроме того, потребуется перенастроить [конфигурацию DNS](https://docs.microsoft.com/azure/private-link/private-endpoint-dns) в кластере для подключения с помощью частной конечной точки. При такой настройке сетевой трафик между клиентом в частной сети и кластером Azure обозреватель данных передается по виртуальной сети и [частной ссылке](https://docs.microsoft.com/azure/private-link/) на магистральную сеть Майкрософт, отключив раскрытие из общедоступного Интернета. В этой статье показано, как создать и настроить частную конечную точку в кластере для запроса (механизма) и приема (Управление данными).

## <a name="prerequisites"></a>Предварительные условия

* Создание [кластера Azure обозреватель данных в виртуальной сети](https://docs.microsoft.com/azure/data-explorer/vnet-create-cluster-portal)
* Отключить политики сети:
  * В виртуальной сети Azure обозреватель данных Cluster отключите [политику службы частной связи](https://docs.microsoft.com/azure/private-link/disable-private-link-service-network-policy).
  * В виртуальной сети с частной конечной точкой, которая может совпадать с виртуальной сетью кластера Azure обозреватель данных, отключите [политику частной конечной точки](https://docs.microsoft.com/azure/private-link/disable-private-endpoint-network-policy).

## <a name="create-private-link-service"></a>Создание службы частной связи

Для безопасной связи со всеми службами в кластере необходимо дважды создать [службу Private Link](https://docs.microsoft.com/azure/private-link/private-link-service-overview) : один раз для запроса (Engine) и один раз для приема (управления данными).
...
1. В области **служба создания частной ссылки** заполните следующие поля:
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | name | азуредатаексплорерплс | Выберите имя, идентифицирующее службу частной связи в группе ресурсов. |
...
1. В области **параметры исходящего трафика** заполните следующие поля:
    ...
    |**Параметр** | **Рекомендуемое значение** | **Описание поля**
    |---|---|---|
    | Load Balancer | Load Balancer модуля или *управления данными* | Выберите подсистему балансировки нагрузки, которая была создана для подсистемы кластеров, подсистема балансировки нагрузки, указывающая на общедоступный IP-адрес подсистемы.  Имя подсистемы балансировки нагрузки будет иметь следующий формат: кукомпуте-{имя_кластера}-Elb <br> *Load Balancer имя управления данными будет иметь следующий формат: кудатамгмт-{имя_кластера}-Elb*|
...
Для безопасной связи со всеми службами в кластере необходимо создать [частную конечную точку](https://docs.microsoft.com/azure/private-link/private-endpoint-overview) дважды: один для запроса (подсистема) и один раз для приема (Управление данными).
...
1. В области **Создание частной конечной точки** заполните следующие поля:
    ...
    **Параметр** | **Рекомендуемое значение** | **Описание поля**
    ...
    | name | азуредатаексплорерпе | Выберите имя, идентифицирующее виртуальную сеть в группе ресурсов.
...
При развертывании кластера Azure обозреватель данных в виртуальной сети мы обновляем [запись DNS](https://docs.microsoft.com/azure/private-link/private-endpoint-dns) , чтобы она указывала на каноническое имя с *привателинк* между именем записи и именем узла зоны. Эта запись обновляется как для механизма, так и для приема (Управление данными). 
