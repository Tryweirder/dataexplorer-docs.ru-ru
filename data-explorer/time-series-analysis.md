...
- Используйте [`make-series`](kusto/query/make-seriesoperator.md) оператора для создания набора из трех временных рядов, где:
    ...
    - `default=0`: укажите метод заполнения отсутствующих ячеек для создания регулярных временных рядов. Альтернативное [`series_fill_const()`](kusto/query/series-fill-constfunction.md) [`series_fill_forward()`](kusto/query/series-fill-forwardfunction.md)использование, [`series_fill_backward()`](kusto/query/series-fill-backwardfunction.md) [`series_fill_linear()`](kusto/query/series-fill-linearfunction.md) и для изменений
