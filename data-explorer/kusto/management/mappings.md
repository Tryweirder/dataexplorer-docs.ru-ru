...
* Если приведенное выше сопоставление является [предварительно созданным](create-ingestion-mapping-command.md) , на него можно `.ingest` ссылаться с помощью команды Control:
...
**Примечание.** Следующий формат сопоставления, без контейнера `Properties` свойств, в настоящее время поддерживается, но может быть нерекомендуемым в будущем.
