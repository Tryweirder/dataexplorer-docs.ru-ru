...
* Если ключ хэш-секции содержит значения, которые сигнификнатли более распространенными, чем другие, например, пустая строка, универсальное значение (например, `null` или `N/A` ), или они представляют сущность (например, `tenant_id` ), которая более распространена в наборе данных, может участвовать в несбалансированном распределении данных между узлами кластера и снижении производительности запросов.
* Если ключ секции даты и времени с равномерным диапазоном имеет большой процент значений из большинства значений в столбце (например, значения DateTime из удаленных или будущих периодов), это может увеличить оверхад процесса секционирования данных и привести к многим небольшим экстентам, которые кластеру необходимо отследить.
