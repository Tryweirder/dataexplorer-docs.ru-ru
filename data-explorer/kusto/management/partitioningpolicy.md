...
Хотя жестко закодированные ограничения на количество таблиц, для которых может быть определена политика, не заданы, во всех дополнительных таблицах в фоновый процесс секционирования данных, выполняющихся на узлах кластера, добавляются дополнительные ресурсы, которые могут потребовать дополнительных ресурсов кластера. Дополнительные сведения см. [ниже](#capacity).
...
* Поскольку процесс секционирования данных приводит к созданию большего количества экстентов, может потребоваться увеличить [емкость объединения экстентов](../management/capacitypolicy.md#extents-merge-capacity) кластера, чтобы процесс [слияния экстентов](../management/extents-overview.md) мог поддерживаться.
* Если это необходимо (например, в случае высокой пропускной способности приема и (или) большого количества таблиц, требующих секционирования), можно увеличить [емкость раздела для экстентов](../management/capacitypolicy.md#extents-partition-capacity) кластера, чтобы разрешить выполнение большего количества параллельных операций секционирования.
