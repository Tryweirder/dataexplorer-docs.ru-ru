...
Применение ключа хэш `string`-секции к столбцу с типом данных в таблице уместно, если в *большинстве* запросов используются фильтры равенства (`==`, `in()`) и (или) статистическое выражение или объединение `string`по определенному типизированному столбцу *большого измерения* (кратность 10 миллионов или выше), например `application_ID`, `tenant_ID` или. `user_ID`
