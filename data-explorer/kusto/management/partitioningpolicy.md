...
* Если ключ хэш-секции содержит значения, которые значительно более распространены, чем другие, например пустая строка, универсальное значение (например, `null` или `N/A` ), или они представляют сущность (например, `tenant_id` ), которая более распространена в наборе данных, может повлиять на несбалансированное распределение данных между узлами кластера и снизить производительность запросов.
* Если ключ секции даты и времени с равномерным диапазоном имеет большой процент значений из большинства значений в столбце (например, значения даты и времени, предшествующие прошлому или будущем), это может увеличить издержки процесса секционирования данных и привести к многим небольшим экстентам, которые кластер должен отследить.
