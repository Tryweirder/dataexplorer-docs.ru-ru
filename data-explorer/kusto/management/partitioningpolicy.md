...
> [!NOTE]
> Применение ключа хэш-секции к `string` столбцу типа в таблице подходит **только** в том случае, если большинство запросов используют фильтры равенства ( `==` , `in()` ) или если они объединены или объединены в определенном `string` столбце *большого размера* (кратность 10 миллионов или выше), например, `application_ID` `tenant_ID` или `user_ID` .
...
> [!NOTE] 
> Применение ключа секции даты и времени с равномерным диапазоном для `datetime` столбца с типом в таблице уместно, **только** если данные, полученные в таблице, вряд ли упорядочиваются в соответствии с этим столбцом.
