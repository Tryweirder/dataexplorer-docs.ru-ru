...
## <a name="extent-ingestion-time"></a>Время приема экстента

Одним из наиболее важных сведений для каждого экстента является время его приема. Это время используется Kusto для:

1. Хранение (экстенты, которые были приняты ранее, будут удалены ранее).
2. Кэширование (недавно полученные экстенты будут температура здания выше).
3. Выборка (при использовании операций запроса, `take`таких как, предпочтительно использовать последние экстенты).
...
Время приема экстента может быть задано одним из трех способов:

1. Как правило, узел, выполняющий прием, задает это значение в соответствии с его локальным часами.
2. Если для таблицы задана **Политика времени приема** , то узел, выполняющий прием, задает это значение в соответствии с локальными часами узла администрирования кластера, гарантируя, что все последующие приемы будут иметь более высокое значение времени приема.
3. Это время может быть задано клиентом. (Это полезно, например, если клиент хочет повторно принять данные и не хочет, чтобы повторно принимаемые данные отображались, как будто они приступили к опозданию, например в целях хранения).    
...
Теги, начинающиеся с **`drop-by:`** префикса, можно использовать для управления тем, с какими другими экстентами следует выполнить слияние. экстенты, имеющие заданный `drop-by:` тег, можно объединить вместе, но они не будут объединяться с другими экстентами. Это позволяет пользователю выдать команду для удаления экстентов в соответствии с их `drop-by:` тегом, например следующей командой:
...
* Не рекомендуется использовать `drop-by` Теги чрезмерного использования. Поддержка удаления данных указанным выше образом предназначена для редко встречающихся событий, не для замены данных на уровне записи, а также для критически важности того, что данные, помеченные таким способом, имеют вид «небольшого объема». Попытка предоставить другой тег для каждой записи или небольшое число записей может привести к серьезному снижению производительности.
