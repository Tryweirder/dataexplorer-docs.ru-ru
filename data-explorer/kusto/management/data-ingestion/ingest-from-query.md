...
|Свойство        |Описание|
|----------------|-----------------------------------------------------------------------------------------------------------------------------|
|`creationTime`   | Значение даты и времени (в формате строки ISO8601), которое будет указано как время создания принятых экстентов данных. Если этот параметр не указан, используется текущее значение (`now()`). Если указано, убедитесь, что свойство `Lookback` в действующей [политике объединения экстентов](../mergepolicy.md) целевой таблицы соответствует указанному значению.|
