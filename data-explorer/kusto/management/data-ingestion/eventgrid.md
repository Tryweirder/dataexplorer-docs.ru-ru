...
При настройке подключения к хранилищу BLOB-объектов к кластеру Azure обозреватель данных укажите свойства целевой таблицы (имя таблицы, формат данных и сопоставление). Эта настройка является маршрутизацией по умолчанию для данных, которая также называется `static routing` .
...
* Kusto выбрано в `Event Hub` качестве типа конечной точки, используемого для передачи уведомлений о событиях хранилища BLOB-объектов. `Event Grid schema`Выбранная схема для уведомлений. Каждый четный концентратор может обслуживать одно подключение.
* Подключение к подписке хранилища BLOB-объектов обрабатывает уведомления типа `Microsoft.Storage.BlobCreated` . Не забудьте выбрать его при создании подписки. Другие типы уведомлений, если они выбраны, игнорируются.
