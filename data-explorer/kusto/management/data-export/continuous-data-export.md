...
* Непрерывный экспорт выполняется в соответствии с настроенным для него периодом времени. Рекомендуемое значение для этого интервала составляет по крайней мере несколько минут в зависимости от задержек, которые вы хотите принять. Непрерывный экспорт *не* предназначен для непрерывной потоковой передачи данных из Kusto. Он выполняется в распределенном режиме, где все узлы экспортируются параллельно.
Если диапазон данных, запрашиваемых каждым запуском, невелик, выходные данные непрерывного экспорта будут иметь много мелких артефактов (число зависит от числа узлов в кластере). 
...
* Если экспортируемый том данных имеет большой размер, настоятельно рекомендуется настроить несколько учетных записей хранения для внешней таблицы, чтобы избежать регулирования хранилища (см. раздел Известные проблемы в документе [Экспорт данных в хранилище](export-data-to-storage.md#known-issues) ).
* Для лучшей производительности кластер ADX и учетные записи хранения должны размещаться в одном регионе Azure.
* Распределение по умолчанию в непрерывном экспорте — `per_node` (все узлы одновременно экспортируются). 
  Этот параметр можно переопределить в свойствах команды непрерывного экспорта. Используйте `per_shard` распространение для увеличения параллелизма (Обратите внимание, что это увеличит нагрузку на учетные записи хранения и может вызывать ограничения регулирования); Используйте `single` (или `distributed` = `false` ), чтобы полностью отключить распространение (это может значительно замедлить процесс непрерывного экспорта). Этот параметр также влияет на количество файлов, создаваемых в каждой итерации непрерывного экспорта (Дополнительные сведения см. в подразделе "Примечания" в [команде "экспорт в внешнюю таблицу](export-data-to-an-external-table.md) ").
* Количество файлов, экспортируемых в каждой итерации непрерывного экспорта, зависит от того, как была секционирована внешняя таблица. Дополнительные сведения см. в подразделе «Примечания» [команды «экспорт во внешнюю таблицу»](export-data-to-an-external-table.md).
Каждая итерация непрерывного экспорта всегда записывает данные в *новые* файлы и никогда не добавляет к существующим. В результате количество экспортируемых файлов также зависит от частоты выполнения непрерывного экспорта ( `intervalBetweenRuns` параметр).
* Влияние непрерывного экспорта на кластер зависит от запроса, в котором выполняется непрерывный экспорт, так как большинство ресурсов (ЦП, память) используются при выполнении запроса. [Команду Показывать команды и запросы](../commands-and-queries.md) можно использовать для оценки потребления ресурсов. Фильтрация по `| where ClientActivityId startswith "RunContinuousExports"` для просмотра команд и запросов, связанных с непрерывным экспортом.
...
| name     | екстерналтабленаме | Запрос | форцедлатенци | интервалбетвинрунс | курсорскопедтаблес         | експортпропертиес                   |
|----------|-------------------|-------|---------------|---------------------|----------------------------|------------------------------------|
...
| IsDisabled          | Логический  | True, если непрерывный экспорт отключен                               |
| IsRunning           | Логический  | True, если непрерывный экспорт выполняется в данный момент                      |
...
| name                | Строка   | Имя непрерывного экспорта                                           |
...
| Выходной параметр  | Type     | Описание                            |
|-------------------|----------|----------------------------------------|
| Timestamp         | Datetime | Метка времени выполнения непрерывного экспорта |
| екстерналтабленаме | Строка   | Имя внешней таблицы             |
...
| Timestamp                   | екстерналтабленаме | Путь             | нумрекордс | сизеинбитес |
|-----------------------------|-------------------|------------------|------------|-------------|
...
| Выходной параметр | Type      | Описание                                         |
|------------------|-----------|-----------------------------------------------------|
| Timestamp        | Datetime  | Метка времени сбоя.                           |
| Операции      | Строка    | ИДЕНТИФИКАТОР операции сбоя.                    |
| name             | Строка    | Имя непрерывного экспорта.                             |
| ластсукцессрун   | Timestamp | Последнее успешное выполнение непрерывного экспорта.   |
...
| Подробности          | Строка    | Сведения об ошибке сбоя.                              |
...
| Timestamp                   | Операции                          | name     | ластсукцессрун              | фаилурекинд | Подробности    |
|-----------------------------|--------------------------------------|----------|-----------------------------|-------------|------------|
