 ...
 * **Создать на основе существующих записей в исходной таблице:** 
      * Создание может занять длительное время, в зависимости от количества записей в исходной таблице. Представление не будет доступно для запросов, пока не будет завершена обратная засыпку.
    ...
    * Отмена процесса обратной обработки возможна с помощью команды [. Отмена операции](#cancel-materialized-view-creation) .
      ...
      > [!IMPORTANT]
      > Использование параметра «обратная засыпку» может занять длительное время для больших исходных таблиц. Если при выполнении этого процесса происходит сбой, повторная попытка не будет выполнена автоматически, и требуется повторное выполнение команды Create. Дополнительные сведения см. в разделе [обратная проматериализованный Просмотр](#backfill-a-materialized-view) .
...
|----------------|-------|---|
|задним числом|bool|Следует ли создать представление на основе всех записей, находящихся в данный момент в *SourceTable* ( `true` ), или создать его "from-now-on" ( `false` ). По умолчанию — `false`.| 
...
* Не включайте преобразования, нормализации, уточняющие запросы в таблицы измерений и другие ресурсоемкие вычисления, которые можно переместить в [политику обновления](../updatepolicy.md) как часть определения материализованных представлений. Вместо этого выполните все эти процессы в политике обновления и выполните агрегирование только в материализованных представлениях.
...
> [!TIP]
> Если требуется оптимальная производительность запросов, но это может допускать некоторую задержку данных, используйте [функцию materialized_view ()](../../query/materialized-view-function.md).

## <a name="backfill-a-materialized-view"></a>Обратная прокладка материализованных представлений

При создании материализованных представлений с помощью `backfill` Свойства материализованные представления будут созданы на основе записей, доступных в исходной таблице (или подмножества этих записей, если `effectiveDateTime` используется). Для больших исходных таблиц обратная засыпку может занять много времени.

* Использование параметра «обратная засыпку» не поддерживается для данных в холодном кэше. Увеличьте период оперативного кэша, если это необходимо, на время создания представления. Для этого может потребоваться горизонтальное масштабирование.

* В процессе обратной отправки данные разбиваются на несколько пакетов, и для получения представления используется несколько операций приема. Повторяются временные сбои, возникающие в процессе обратной отправки, но если все повторные попытки исчерпаны, может потребоваться повторное выполнение команды CREATE вручную.

* Существует несколько свойств, которые можно попробовать изменить, если возникают сбои при создании представления:

    * `MaxSourceRecordsForSingleIngest` — по умолчанию число исходных записей в каждой операции приема во время обратной отправки составляет 2 000 000 записей на узел. Это значение по умолчанию можно изменить, задав для этого свойства нужное число записей (значением является _Общее_ число записей в каждой операции приема). Уменьшение этого значения может оказаться полезным, если при создании происходит сбой с пределами памяти или ожиданиями запросов. Увеличение этого значения может ускорить создание представления, предполагая, что кластер способен выполнять статистическую функцию для большего числа записей, чем по умолчанию.

    * `Concurrency` — операции приема, выполняемые как часть процесса обратной отправки, выполняются параллельно. По умолчанию параллелизм имеет значение `min(number_of_nodes * 2, 5)` . Это свойство можно задать, чтобы увеличить или уменьшить параллелизм. Увеличение этого значения рекомендуется, только если ЦП кластера низкий, так как это может оказать значительное влияние на использование ЦП кластера.

  Например, следующая команда отправит материализованный вид из `2020-01-01` , с максимальным числом записей в каждой операции приема `3 million` записей и выполнит операции приема с параллелизмом `2` : 
...
Процесс создания не может быть прерван немедленно. Команда Cancel сигнализирует о прекращении материализации, а создание периодически проверяет, была ли запрошена отмена. Команда Cancel ожидает максимальный период в 10 минут до тех пор, пока процесс создания материализованных представлений не будет отменен и снова сообщит об успешном завершении. Даже если отмена завершилась неудачей в течение 10 минут, а команда Cancel сообщает об ошибке, то, возможно, материализованный вид будет прерван позже в процессе создания. [`.show operations`](../operations.md#show-operations)Команда сообщит, была ли операция отменена. `cancel operation`Команда поддерживается только для отмены создания материализованных представлений, а не для отмены каких-либо других операций.
