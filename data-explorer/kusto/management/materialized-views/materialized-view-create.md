...
ms.locfileid: "91057257"
---
# <a name="create-materialized-view"></a>. Создание материализованных представлений
...
Общие сведения и рекомендации по созданию материализованных представлений см. в разделе [Создание материализованных представлений](materialized-view-overview.md#create-a-materialized-view).

Требуются разрешения [администратора базы данных](../access-control/role-based-authorization.md) .

Материализованный представление всегда основано на единственном `fact table` , а также может ссылаться на один или несколько [`dimension tables`](../../concepts/fact-and-dimension-tables.md) . Ограничения на соединение с таблицами измерений в материализованных представлениях см. в разделе [Свойства](#properties). Общие ограничения см. [в разделе ограничения на создание материализованных представлений](materialized-view-overview.md#limitations-on-creating-materialized-views). 

* Отследите процесс создания с помощью команды [. показ операций](../operations.md#show-operations) .
* Отмените процесс создания, выполнив команду [. Cancel Operation](#cancel-materialized-view-creation) .
...
|----------------|-------|---|
|ViewName|Тип String|Имя материализованных представлений. Имя представления не может конфликтовать с именами таблиц или функций в той же базе данных и должно соответствовать [правилам именования идентификаторов](../../query/schema-entities/entity-names.md#identifier-naming-rules). |
|SourceTableName|Тип String|Имя исходной таблицы, в которой определено представление.|
|Запрос|Тип String|Запрос материализованных представлений. Дополнительные сведения см. в разделе [Query](#query-argument).|
...
|Свойство|Тип|Описание | Примечания |
|----------------|-------|---|---|
|задним числом|bool|Следует ли создать представление на основе всех записей, находящихся в данный момент в *SourceTable* ( `true` ), или создать его "from-now-on" ( `false` ). Значение по умолчанию — `false`.| Команда должна быть `async` , и представление не будет доступно для запросов до завершения создания. В зависимости от объема данных, которые необходимо просуммить, создание с помощью обратного отсчета может занять длительное время. Он намеренно «замедлит», чтобы убедиться, что он не потребляет слишком много ресурсов кластера. См. [Описание обратная засыпку](materialized-view-overview.md#create-a-materialized-view) |
...
|дименсионтаблес|Массив|Разделенный запятыми список таблиц измерений в представлении.|  Таблицы измерений должны быть явно вызваны в свойствах представления. Для соединений и уточняющих запросов с таблицами измерений следует использовать рекомендации по [запросам](../../query/best-practices.md). См. раздел [соединение с таблицей измерения](#join-with-dimension-table).
|аутаупдатесчема|bool|Указывает, следует ли автоматическое обновление представления в исходной таблице. Значение по умолчанию — `false`.| `autoUpdateSchema`Параметр допустим только для представлений типа `arg_max(Timestamp, *)`  /  `arg_min(Timestamp, *)`  /  `any(*)` (только если аргумент Columns имеет значение `*` ). Если для этого параметра задано значение true, изменения в исходной таблице будут автоматически отражены в материализованных представлениях. При использовании этого параметра поддерживаются не все изменения в исходной таблице. Дополнительные сведения см. в разделе [. изменение материализованных представлений](materialized-view-alter.md). |
...
> [!WARNING]
> Использование `autoUpdateSchema` может привести к необратимой потере данных при удалении столбцов исходной таблицы. Представление будет отключено, если оно не имеет значение `autoUpdateSchema` , и в исходную таблицу внесено изменение, которое приводит к изменению схемы материализованным представлением. Если проблема устранена, повторно включите материализованный представление с помощью команды [Enable материализованный Просмотр](materialized-view-enable-disable.md) . 
>
>Этот процесс часто используется при использовании `arg_max(Timestamp, *)` и добавлении столбцов в исходную таблицу. Избегайте сбоя, определив запрос представления как или с `arg_max(Timestamp, Column1, Column2, ...)` помощью `autoUpdateSchema` параметра. 

### <a name="join-with-dimension-table"></a>Соединение с таблицей измерения
...
1. Создание материализованных представлений с помощью параметра «обратная засыпку» с использованием `async` следующих средств:
...
|Операция|Тип String|Тип операции.
