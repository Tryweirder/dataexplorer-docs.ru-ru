...
Настройте стратегию запросов в случайном порядке с `hint.strategy = shuffle` помощью `hint.shufflekey = <key>`параметра запроса или.

Определите [политику секционирования данных](../management/partitioningpolicy.md) для таблицы. 

Задайте `shufflekey` в качестве ключа хэш-секции таблицы для повышения производительности, так как объем данных, необходимых для перемещения между узлами кластера, уменьшается.
...
Если у оператора, переданного в случайном порядке, есть другие `summarize` операторы `join`, такие как или, запрос станет более сложным, а затем подсказка. Стратегия = случайное перемещение не будет применена.
...
`summarize` Оператор находится в левой части `join` оператора. Этот оператор будет группировать по подмножеству `join` ключей, которое в нашем случае имеет значение `ActivityId`. Таким же блок `summarize` будет группироваться по ключу `ActivityId`, а данные будут секционированы по составному ключу [`ActivityId`, `numeric_column`].
...
В этом примере предполагается, что хэш-функция, используемая `binary_xor(hash(key1, 100) , hash(key2, 100))`для составного ключа:
...
Составной ключ для обеих записей был сопоставлен с разными секциями (56 и 65), но эти две записи имеют одинаковое значение `ActivityId`. `summarize` Оператор в левой части объекта `join` принимает аналогичные значения столбца `ActivityId` в одну и ту же секцию. При выполнении этого запроса будут получены неверные результаты.

Эту проблему можно решить, используя `hint.shufflekey` , чтобы указать клавишу "случайный" в `hint.shufflekey = ActivityId`соединении с. Этот ключ является общим для всех операторов, поддерживающих случайное перемещение.
В этом случае перетасовывание является надежным, так как `join` и `summarize` , и в случайном порядке с одним и тем же ключом. Таким образом, все аналогичные значения будут находиться в одной секции, и результаты будут верными:
...
При использовании стратегии `summarize` случайного использования запрос завершается через ~ 7 секунд, а пиковое использование памяти составляет 0,43 Гб:
...
При использовании стратегии `join` случайного использования запрос завершается через ~ 4 секунды, а пиковое использование памяти составляет 0,3 ГБ:
...
Попытка выполнения тех же запросов в наборе данных большего размера, `join` где левая часть — 150M, а кратность ключа — 148M. Правая часть `join` — 1,5 б, а количество элементов ключа — ~ 100 млн.

Запрос с стратегией по `join` умолчанию обращается к Kusto ограничениям и времени ожидания через 4 минуты.
При использовании стратегии `join` случайного использования запрос завершается через ~ 34 секунд, а пиковое использование памяти — на 1,23 ГБ.
