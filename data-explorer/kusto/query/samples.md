...
|name|Город|SessionId|Отметка времени|
|---|---|---|---|
...
|Город|SessionId|StartTime|StopTime|Длительность|
|---|---|---|---|---|
...
Затем мы можем добавить код для подсчета длительностей в ячейках удобного размера. Мы сделали незначительную настройку для линейчатой диаграммы, так что мы разделены `1s` на, чтобы преобразовать интервалы времени в числа. 
...
4. Все с `Count` `0` .
5. Наконец, мы используем `summarize` оператор для группирования ячеек из исходного (левого или внешнего) аргумента в `union` и ячейки из внутреннего аргумента в него (а именно строки с пустыми ячейками). Это гарантирует, что выход будет содержать по одной строке на ячейку, значение которой равно нулю или является исходным числом.  
