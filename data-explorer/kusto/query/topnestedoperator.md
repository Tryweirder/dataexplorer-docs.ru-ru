...
* *`ConstExpr`*: Если указано, то для каждой записи уровня иерархии будет добавлено значение, которое является агрегатом для всех записей, которые не сделали это в верхней части.
...
Для случаев, когда распределение агрегата значительно отличается от универсального, ограничьте количество уникальных возвращаемых значений (с помощью *N*) и используйте `with others=` параметр *ConstExpr* , чтобы получить указание для "веса" всех остальных вариантов.
...
|Область|aggregated_State|Источник|aggregated_Source|ендлокатион|aggregated_EndLocation|
|---|---|---|---|---|---|
...
|Область|aggregated_State|Источник|aggregated_Source|ендлокатион|aggregated_EndLocation|EventType|
|---|---|---|---|---|---|---|
...
|Область|Источник|ендлокатионс|ендлокатионсумс|индиЦиес|
|---|---|---|---|---|
