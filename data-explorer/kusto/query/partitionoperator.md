...
* *Партитионпараметерс*: ноль или более (разделенных пробелами) параметров в виде: *имя* `=` *значение* , определяющее поведение оператора. Поддерживаются следующие параметры:
  ...
  |`hint.concurrency`|*Число*|Описывает систему, сколько одновременных вложенных запросов `partition` оператора должно выполняться параллельно. *По умолчанию*: количество ядер ЦП на одном узле кластера (от 2 до 16).|
  |`hint.spread`|*Число*|Подсказка системы, сколько узлов должно использоваться при выполнении параллельных `partition` вложенных запросов. *Значение по умолчанию*: 1.|
...
```
|EventType|State|События|Травмах|
|---|---|---|---|
...
Иногда полезно (понизить производительность) выполнять сложный вложенный запрос к непересекающимся секциям данных в стиле "схема/уменьшение". В приведенном ниже примере показано, как создать ручное распределение агрегата для 10 секций.
