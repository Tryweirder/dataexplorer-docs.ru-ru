---
title: Секционирование и составление промежуточных результатов агрегатов — Azure обозреватель данных
...
Значение, если необходимо вычислить число уникальных пользователей каждый день за последние семь дней. Вы можете запустить `summarize dcount(user)` один раз в день с диапазоном, отфильтрованным за последние семь дней. Этот метод неэффективен, поскольку при каждом запуске вычисления существует шесть дней с предыдущим вычислением. Можно также вычислить статистическое выражение для каждого дня, а затем объединить эти статистические выражения. Этот метод требует «запомнить» последние шесть результатов, но гораздо эффективнее.
...
При принятии этого значения в таблицу перед применением этого типа политики будет принимать значение NULL:
...
Если необходимо получить окончательные результаты этих значений, запросы могут использовать слияния ХЛЛ/тдижест: [`hll-merge()`](hll-merge-aggfunction.md) / [`tdigest_merge()`](tdigest-merge-aggfunction.md) , а после получения Объединенных значений [`percentile_tdigest()`](percentile-tdigestfunction.md)  /  [`dcount_hll()`](dcount-hllfunction.md) для получения окончательного результата или процентили могут быть вызваны эти объединенные значения `dcount` .
...
Это должно быть более производительно, а запрос выполняется более чем с меньшей таблицей (в этом примере первый запрос выполняется свыше ~ 215M записей, а вторая — только для 32 записей).
...
При использовании функций [`hll()`](hll-aggfunction.md) , [`hll_merge()`](hll-merge-aggfunction.md) и [`dcount_hll()`](dcount-hllfunction.md) эквивалентный запрос завершается через ~ 1,3 секунд и показывает, что `hll` функции ускоряет выполнение запроса выше в ~ 14 раз:
...
> [!NOTE] 
> Результаты запросов не 100% в соответствии с ошибками `hll` функций. ( Дополнительные сведения об ошибках см. в разделе [`dcount()`](dcount-aggfunction.md) .
