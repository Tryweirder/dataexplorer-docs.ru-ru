...
|Поле  |Определение  |Пример  |
|---------|---------|---------|
| **view** | Может использоваться только в лямбда-функции без параметров, не имеющей аргументов. Указывает, что связанное имя будет включено, если все таблицы являются запросами. | Например, при использовании `union *`.|
| ***TabularArguments** _ | Список аргументов формального табличного выражения. 
