---
title: Оператор make-Series в Azure обозреватель данных | Документация Майкрософт
...
ms.locfileid: "82618447"
---
# <a name="make-series-operator"></a>Оператор make-series

Создать ряд указанных агрегированных значений по указанной оси. 
...
* *DefaultValue:* Значение по умолчанию, которое будет использоваться вместо отсутствия значений. Если нет строки с конкретными значениями *AxisColumn* и *GroupExpression* , то в результатах соответствующий элемент массива будет назначен с *DefaultValue*. Если `default =` значение *DefaultValue* опущено, предполагается значение 0. 
* *Статистическая обработка:* Вызов [агрегатной функции](make-seriesoperator.md#list-of-aggregation-functions) , такой как `count()` или `avg()`, с именами столбцов в качестве аргументов. См. [список статистических функций](make-seriesoperator.md#list-of-aggregation-functions). Обратите внимание, что с `make-series` оператором можно использовать только статистические функции, возвращающие числовые результаты.
* *AxisColumn:* Столбец, в котором будет упорядочен ряд. Ее можно рассматривать как временную шкалу, `datetime` но помимо любых числовых типов.
* *Начало*: (необязательно) значение нижней границы *AxisColumn* для каждого ряда будет построено. *Start*, *End* и *Step* используются для построения массива значений *AxisColumn* в заданном диапазоне и с помощью указанного *шага*. Все *статистические* значения упорядочиваются соответственно этому массиву. Этот массив *AxisColumn* также является последним выходным столбцом в выходных данных с тем же именем, что и *AxisColumn*. Если *Начальное* значение не указано, то начало — это первая ячейка (Step), которая содержит данные в каждом ряду.
* *конец*: (необязательно) значение высокой границы (неинклюзивное) для *AxisColumn*, последний индекс временного ряда меньше этого значения (и будет *начинаться* плюс целое *число, которое меньше* , чем *End*). Если *Конечное* значение не указано, оно будет верхней границей последней ячейки (шага), которая содержит данные по каждому ряду.
* *шаг*. разница между двумя последовательными элементами массива *AxisColumn* (т. е. размером ячейки).
...
Наконец, строки из промежуточного результата, сгруппированные в группы с одинаковыми значениями `by` выражений, и все агрегированные значения упорядочиваются в массивы ( `dynamic` значения типа). Для каждого агрегата существует один столбец, содержащий массив с тем же именем. Последний столбец в выходных данных функции Range со всеми значениями *AxisColumn* . Его значение повторяется для всех строк. 

Обратите внимание, что вследствие заполнения отсутствующих ячеек значением по умолчанию результирующая Сводная таблица имеет то же количество ячеек (т. е. агрегированные значения) для всех рядов.  
...
*T* `| make-series` [*столбец* `=`] *агрегирование* [`default` `=` *DefaultValue*]`,` [...] `on` *AxisColumn* `,` *step* `,` *start* `)` *stop* `=` *GroupExpression* *Column* запустить прерывать шаг`by` [[столбец] GroupExpression [...]]`,` `in` `range(`
...
* Группирования. ось индекса создается с помощью bin (), а не bin_at (), что означает, что *Запуск* не обязательно должен быть включен в созданный ряд.
...
|[AVG ()](avg-aggfunction.md)|Среднее значение ретунс в группе|
|[Count ()](count-aggfunction.md)|Возвращает число групп|
|[countif()](countif-aggfunction.md)|Возвращает Count с предикатом группы|
...
* Примечание. функции интерполяции по умолчанию `null` предполагают отсутствие значения. Поэтому рекомендуется указывать `default=` *Double*(`null`) в `make-series` , если предполагается использовать функции интерполяции для ряда. 

## <a name="example"></a>Пример
  
 Таблица, в которой отображаются массивы чисел и средние цены каждого из фруктов каждого из поставщиков, упорядоченные по метке времени с указанным диапазоном. В выходных данных есть строка для каждого отдельного сочетания фруктов и поставщика. Выходные столбцы показывают объем фруктов, поставщик и массивы: количество, среднее и всю строку времени (от 2016-01-01 до 2016-01-10). Все массивы сортируются по соответствующей метке времени, а все пробелы заполняются значениями по умолчанию (0 в этом примере). Все остальные входные столбцы игнорируются.
...
При `kind=nonempty` использовании `make-series` в будет получен непустой результат значений по умолчанию:
