...
* Примечание. функции интерполяции по умолчанию предполагают `null` отсутствие значения. Поэтому рекомендуется указывать `default=` *Double*( `null` ) в `make-series` , если предполагается использовать функции интерполяции для ряда. 
...
`kind=nonempty`При использовании в `make-series` будет получен непустой результат значений по умолчанию:
