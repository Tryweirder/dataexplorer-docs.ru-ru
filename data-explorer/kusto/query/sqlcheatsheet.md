---
title: Перевод запроса на запрос Kusto - Azure Data Explorer Документы Майкрософт
description: В этой статье описывается перевод запроса Kusto в Azure Data Explorer.
...
ms.locfileid: "81507452"
---
# <a name="sql-to-kusto-query-translation"></a>Перевод запроса на Кусто

Kusto поддерживает подмножество языка S'L. Полный список неподдерживаемых функций можно ознакомиться со списком [известных проблем, известных в S'L.](../api/tds/sqlknownissues.md)

Основным языком для взаимодействия с Kusto является КЗЛ (Язык кикс-запросов), и для того, чтобы сделать переход и опыт обучения проще, вы можете использовать сервис Kusto для перевода запросов на КЗЛ. Этого можно достичь, отправив запрос на обслуживание Kusto, предварительно зафиксировав его глаголом 'EXPLAIN'.
...
|Запрос|
|---|
|StormEvents<br>| суммировать C'count()<br>| Проект C|

## <a name="sql-to-kusto-cheat-sheet"></a>СЗЛ в кусто шпаргалка

В таблице ниже показаны выборочные запросы в СЗЛ и их эквиваленты КЗЛ.

|Категория |Запросы SQL |Кусто Запрос
|---|---|---
...
Сравнение (булеан) |<code>SELECT * FROM dependencies<br>WHERE !(success)</code> |<code>dependencies<br>&#124; where success == "False"</code>
...
Псевдонимы колонны, расширение |<code>SELECT operationName as Name, AVG(duration) as AvgD FROM dependencies<br>GROUP BY name</code> |<code>dependencies<br>&#124; summarize AvgD = avg(duration) by operationName<br>&#124; project Name = operationName, AvgD</code>
...
Топ n по меркам |<code>SELECT TOP 100 name, COUNT(*) as Count FROM dependencies<br>GROUP BY name<br>ORDER BY Count DESC</code> |<code>dependencies<br>&#124; summarize Count = count() by name<br>&#124; top 100 by Count desc</code>
...
Имея |<code>SELECT COUNT(\*) FROM dependencies<br>GROUP BY name<br>HAVING COUNT(\*) > 3</code> |<code>dependencies<br>&#124; summarize Count = count() by name<br>&#124; where Count > 3</code>|