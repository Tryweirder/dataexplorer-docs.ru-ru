---
title: quantize_udf () — обозреватель данных Azure
description: В этой статье описывается определяемая пользователем функция quantize_udf () в Azure обозреватель данных.
author: orspod
ms.author: orspodek
...
ms.locfileid: "89557928"
---
# <a name="quantize_udf"></a>quantize_udf ()


Функция `quantize_udf()` заячеекет столбцы метрик. Он куантизес столбцы метрик в метки категорий, основанные на алгоритме K-средних.
...
`T | invoke quantize_udf(`*num_bins* `,` *in_cols* `,` *out_cols* `,` *метки*`)`
...
* `quantize_udf()` — Это определяемая пользователем функция. Можно либо внедрить его код в запрос, либо установить в базе данных:
...
* `quantize_udf()` — Это [Табличная функция](../query/functions/user-defined-functions.md#tabular-function), применяемая с помощью [оператора Invoke](../query/invokeoperator.md)
