---
title: quantize_lf () — обозреватель данных Azure
description: В этой статье описывается определяемая пользователем функция quantize_lf () в Azure обозреватель данных.
author: orspod
ms.author: orspodek
...
ms.locfileid: "89614945"
---
# <a name="quantize_lf"></a>quantize_lf ()


Функция `quantize_lf()` заячеекет столбцы метрик. Он куантизес столбцы метрик в метки категорий, основанные на алгоритме K-средних.

> [!NOTE]
> `quantize_lf()` — Это [определяемая пользователем функция (UDF)](../query/functions/user-defined-functions.md). Эта функция содержит встроенный язык Python и требует [включения подключаемого модуля Python ()](../query/pythonplugin.md#enable-the-plugin) в кластере. Дополнительные сведения см. в разделе [Использование](#usage).

## <a name="syntax"></a>Синтаксис

`T | invoke quantize_lf(`*num_bins* `,` *in_cols* `,` *out_cols* `,` *метки*`)`

## <a name="arguments"></a>Аргументы

* *num_bins*: требуемое число ячеек.
* *in_cols*: динамический массив, содержащий имена столбцов для квантуем.
* *out_cols*: динамический массив, содержащий имена соответствующих выходных столбцов для значений binned.
* *метки*: динамический массив, содержащий имена меток. Этот параметр является необязательным. Если *метки* не заданы, будут использоваться диапазоны ячеек.

## <a name="usage"></a>Использование

`quantize_lf()` определяемая пользователем [Табличная функция](../query/functions/user-defined-functions.md#tabular-function), применяемая с помощью [оператора Invoke](../query/invokeoperator.md). Можно либо внедрить его код в запрос, либо установить в базе данных. Существует два варианта использования: прямое и постоянное использование. Примеры см. на следующих вкладках.
