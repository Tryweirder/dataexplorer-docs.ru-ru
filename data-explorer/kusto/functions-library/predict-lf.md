---
title: predict_lf () — обозреватель данных Azure
description: В этой статье описывается определяемая пользователем функция predict_lf () в Azure обозреватель данных.
author: orspod
ms.author: orspodek
...
ms.locfileid: "89614938"
---
# <a name="predict_lf"></a>predict_lf ()

Функция `predict_lf()` прогнозирует использование существующей обученной модели машинного обучения. Эта модель была создана с помощью [Scikit-учиться](https://scikit-learn.org/stable/), сериализована в строку и сохранена в стандартной таблице обозреватель данных Azure.

> [!NOTE]
> `predict_lf()` — Это [определяемая пользователем функция (UDF)](../query/functions/user-defined-functions.md). Эта функция содержит встроенный язык Python и требует [включения подключаемого модуля Python ()](../query/pythonplugin.md#enable-the-plugin) в кластере. Дополнительные сведения см. в разделе [Использование](#usage).
...
`predict_lf()` определяемая пользователем [Табличная функция](../query/functions/user-defined-functions.md#tabular-function) , применяемая с помощью [оператора Invoke](../query/invokeoperator.md). Можно либо внедрить его код в запрос, либо установить в базе данных. Существует два варианта использования: прямое и постоянное использование. Примеры см. на следующих вкладках.
