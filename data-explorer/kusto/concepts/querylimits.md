...
* Значение по умолчанию для ограничения на параллелизм запросов зависит от SKU виртуальных машин в кластере, на которых эти запросы выполняются, и рассчитывается по формуле `Cores-Per-Node x 10`.
  * Например, для кластера с виртуальными машинами D14v2 с 16 виртуальными ядрами на каждой, значение по умолчанию для ограничения на параллелизм запросов будет равно `16 cores x10 = 160`.
* Такое значение по умолчанию можно изменить, настроив [политику регулирования запросов](../management/query-throttling-policy.md). 
  * Фактическое число запросов, которые можно одновременно выполнять в кластере, зависит от различных факторов. Основные факторы — это SKU виртуальных машин в кластере, доступные ресурсы кластера и шаблоны запросов. Политику регулирования запросов можно настроить на основе результатов нагрузочных тестов, выполняемых для аналогичных шаблонов запросов в рабочей среде.
...
Существует несколько стратегий для устранения такой ошибки.
...
* Уменьшите размер результирующего набора, реализовав в самом запросе последующую обработку, например агрегирование. Такая стратегия подходит для сценариев, в которых выходные данные запроса передаются в другую систему обработки с последующим агрегированием.
...
Результат усечения также можно сделать еще более точным с помощью значений `truncationmaxsize` (максимальный размер данных в байтах, по умолчанию 64 МБ) и `truncationmaxrecords` (максимальное число записей, по умолчанию 500 000). Например, в следующем запросе усечение результатов выполняется при достижении 1105 записей или размера в 1 МБ (в зависимости от того, что произойдет раньше).
...
* Если задано свойство `notruncation` и при этом задан параметр `truncationmaxsize`, `truncationmaxrecords` или `query_take_max_records`, то `notruncation` игнорируется.
...
В настоящее время отсутствует переключатель, позволяющий увеличить максимальный размер набора строк.
...
* Если вы отправляете запрос с помощью Kusto.Explorer, выберите **Средства** &gt; **Параметры** _ &gt; _ *Подключения** &gt; **Время ожидания сервера запросов**.
...
Kusto позволяет указать два [свойства клиентского запроса](../api/netfx/request-properties.md) при его выполнении: *query_fanout_threads_percent* и *query_fanout_nodes_percent*.
...
Первое свойство, *query_fanout_threads_percent*, управляет коэффициентом разветвления для использования потоков. Если задать значение 100 %, кластер назначит для выполнения запросов все ЦП на каждом узле, например 16 ЦП в кластере, развернутом на узлах Azure D14. Если задать значение 50 %, будет использоваться половина ЦП, и т. д. Числа округляются до целых, поэтому для ЦП можно задать значение 0. Второе свойство, *query_fanout_nodes_percent*, управляет тем, сколько узлов запросов в кластере необходимо использовать для каждой операции распределения вложенных запросов. Она действует аналогичным образом.
