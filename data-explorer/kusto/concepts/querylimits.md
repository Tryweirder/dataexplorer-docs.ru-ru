...
Превышение ограничения на параллелизм запросов приведет к следующему:
* команды, отклоненные из-за политики ограничения частоты запросов, выдадут исключение `ControlCommandThrottledException` (код ошибки — 429);
* запросы, отклоненные из-за политики ограничения частоты запросов, выдадут исключение `QueryThrottledException` (код ошибки — 429).
