...
Существует риск, что запросы будут монопольно заключаться в ресурсах службы без ограничений. Kusto предоставляет ряд встроенных функций защиты в виде ограничений по умолчанию для запросов.
...
* Значение по умолчанию можно изменить, создав запрос в службу поддержки. В будущем этот элемент управления также будет предоставлен с помощью команды Control.

## <a name="limit-on-result-set-size-result-truncation"></a>Ограничение на размер результирующего набора (усечение результата)

**Усечение результатов** — это ограничение, установленное по умолчанию для результирующего набора, возвращаемого запросом. Kusto ограничивает число записей, возвращаемых клиенту, до **500 000**, а общая память для этих записей — **64 МБ**. При превышении какого-либо из этих ограничений запрос завершается ошибкой "частичный сбой запроса". Превышение общей памяти приведет к возникновению исключения с сообщением:
...
* Попросите службы отключить этот предел запросов.
...
В настоящее время клиентские библиотеки Kusto предполагают наличие этого предела. Хотя это ограничение можно увеличить без пределов, в конечном итоге вы будете обращаться к ограничениям клиентов, которые в настоящее время не настраиваются.

Клиенты, которые не хотят извлекать все данные в одной операции, могут попробовать следующие решения:
* Переключить некоторые пакеты SDK на режим потоковой передачи (Streaming = true Property для Кустоконнектионстрингбуилдер)
* Переключиться на API .NET v2

Сообщите команде Kusto о том, что вы выпустили эту ошибку, поэтому мы можем повысить приоритет клиента потоковой передачи.
...
При удалении этих ограничений сначала определите, действительно ли вы получаете какое бы то ни было значение. В частности, удаление предельного усечения результатов означает, что вы собираетесь переместить данные большого объема из Kusto.
Можно удалить предел усечения результатов для целей экспорта с помощью `.export` команды или для последующего агрегирования, в этом случае следует рассмотреть возможность статистической обработки с помощью Kusto.
Сообщите команде Kusto о том, что у вас есть бизнес-сценарий, который не может быть удовлетворен одним из предлагаемых решений.  
...
Ограничения превышаются из-за запросов с длинными списками бинарных операторов, Объединенных в цепочку. Например:
