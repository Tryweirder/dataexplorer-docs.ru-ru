...
Если задать значение 100 %, кластер назначит для выполнения запросов все ЦП на каждом узле, например 16 ЦП в кластере, развернутом на узлах Azure D14.
Если задать значение 50 %, будет использоваться половина ЦП, и т. д.
Числа округляются до целых, поэтому для ЦП можно задать значение 0.
...
Если свойство `query_fanout_nodes_percent` или `query_fanout_threads_percent` задано несколько раз, например в свойствах клиентского запроса и с помощью инструкции `set`, применяется *меньшее* значение для каждого свойства.
