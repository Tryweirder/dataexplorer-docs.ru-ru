---
title: Kusto. Explorer — средство Azure обозреватель данных | Документация Майкрософт
...
* Кроме того, вы можете получить доступ к кластеру Kusto `https://<your_cluster>.kusto.windows.net`в браузере по адресу:. Замените <your_cluster> именем кластера Azure обозреватель данных.
...
Уникальное `AAD_TENANT_OF_CLUSTER` название — это доменное имя или идентификатор клиента AAD (GUID) клиента AAD, в котором размещается кластер (обычно это доменное имя Организации, владеющее кластером, например `contoso.com`), а USER_DOMAIN является идентификатором пользователя, приглашаемого в этот клиент (например, `joe@fabrikam.com`). 
...
1. Убедитесь, что расширение файла соответствует содержимому файла. Например:
...
1. Выберите **Экспорт в буфер обмена** (или `Ctrl+Shift+C`нажмите клавишу).
...
Если вы планируете использовать одно и то же значение в нескольких запросах или на нескольких вкладках, это будет сложно изменить. Однако Kusto. Explorer поддерживает параметры запроса. Параметры обозначаются {} квадратными скобками. Пример: `{parameter1}`
...
Длина запросов ограничена длиной не более 2000 символов из-за ограничений Internet Explorer (это ограничение является приблизительным, так как оно зависит от длины имени кластера https://support.microsoft.com/kb/208427 и базы данных). чтобы уменьшить вероятность того, что вы достигли предельного значения, ознакомьтесь с [более короткими ссылками](#getting-shorter-links)ниже.

Формат URI: https://<ClusterCname>. kusto.Windows.NET/<DatabaseName>? запрос =<QueryToExecute>

Например:  https://help.kusto.windows.net/Samples?query=StormEvents+%7c+limit+10
 
Этот универсальный код ресурса (URI) откроет Kusto. `help` Explorer, подключится к кластеру Kusto и запустит указанный запрос к `Samples` базе данных. Если уже запущен экземпляр Kusto. Explorer, запущенный экземпляр откроет новую вкладку и запустит в ней запрос.)
...
Например, чтобы запустить Kusto. Explorer с файлом скрипта с именем `c:\temp\script.kql` и настроить для взаимодействия с кластером `help`, `Samples`базу данных, используйте следующую команду:
...
|`query`   |Выполняемый запрос (в кодировке Base64). Если значение пустое `querysrc`, используйте.          |
|`querysrc`|URL-адрес файла или большого двоичного объекта, содержащего запрос для выполнения `query` (если пуст).|
...
1. Выберите каждый элемент, имя которого начинается `Kusto.Explorer`с.
1. Выберите **Удалить**.
...
1. Выполните команду `appwiz.cpl`.
...
Данные, хранящиеся в приложении Kusto. Explorer, можно найти `C:\Users\\[your alias]\AppData\Local\Kusto.Explorer`здесь:.
