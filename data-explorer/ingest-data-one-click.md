...
* На вкладке **Schema** (Схема) вы можете сделать следующее:
...
* При приеме данных из источников, которые **не являются контейнерами**:
    * Прием выполняется немедленно.
...
* Если источник данных является **контейнером**:
    ...
    * Вы можете выбрать вариант **Create continuous ingestion** (Создать непрерывный прием) и настроить [непрерывный прием с помощью Сетки событий](one-click-ingestion-new-table.md#continuous-ingestion---container-only).
