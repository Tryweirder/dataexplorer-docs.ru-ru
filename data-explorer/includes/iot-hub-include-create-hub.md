...
1. Заполните следующие поля на вкладке **Основные сведения**.
   ...
   - **Группа ресурсов**. Выберите группу ресурсов или создайте новую. Чтобы создать новую, щелкните **Создать** и введите нужное имя. Чтобы выбрать существующую группу ресурсов, щелкните ее. Дополнительные сведения о группах ресурсов см. в статье об [управлении группами ресурсов в Azure Resource Manager](/azure/azure-resource-manager/management/manage-resource-groups-portal).

   - **Регион**. Выберите регион, в котором будет расположен концентратор. Выберите ближайшее к вам расположение. Некоторые функции, такие как [потоки устройств Центра Интернета вещей](/azure/iot-hub/iot-hub-device-streams-overview), доступны только в определенных регионах. Для функций с такими ограничениями необходимо выбрать один из поддерживаемых регионов.
...
1. По завершении выберите **Next: Size and scale** (Далее: размер и масштаб), чтобы продолжить создание концентратора.
   ...
   Здесь вы можете принять настройки по умолчанию. При необходимости можете изменить любое из следующих полей: 
    ...
    Дополнительные сведения о других параметрах уровня см. в статье [Масштабирование решения для Центра Интернета вещей](/azure/iot-hub/iot-hub-scaling).
...
1.  По завершении выберите **Next: Теги**, чтобы перейти к следующему экрану.
    ...
    Теги — это пары "имя — значение". Один и тот же тег можно присвоить нескольким ресурсам и группам ресурсов для их классификации и объединения счетов. Дополнительные сведения см. в статье об [использовании тегов для организации ресурсов в Azure](/azure/azure-resource-manager/management/tag-resources).
