...
ms.locfileid: "93349500"
---
# <a name="visualize-data-with-azure-data-explorer-dashboardspreview"></a>Визуализация данных с помощью панелей мониторинга обозреватель данных Azure (Предварительная версия)
...
> [!IMPORTANT]
> Ваши данные безопасны. Панели мониторинга и метаданные, связанные с панелью мониторинга, о пользователях шифруются неактивных.
...
Используйте меню поделиться, чтобы [предоставить разрешения](#grant-permissions) на панель мониторинга, [изменить уровень разрешений пользователя](#change-a-user-permission-level)и [поделиться ссылкой на панель мониторинга](#share-the-dashboard-link).

> [!IMPORTANT]
> Для доступа к панели мониторинга средство просмотра панели мониторинга должно иметь следующее:
> * Ссылка на панель мониторинга для доступа
> * Разрешения панели мониторинга
> * Доступ к базовой базе данных в кластере Azure обозреватель данных  

1. Выберите пункт меню **поделиться** на верхней панели панели мониторинга.
1. В раскрывающемся списке выберите **Управление разрешениями** . 

    :::image type="content" source="media/adx-dashboards/share-dashboard.png" alt-text="Раскрывающийся список общей панели мониторинга":::

### <a name="grant-permissions"></a>Предоставить разрешения

Чтобы предоставить разрешения пользователю на панели разрешений панели **мониторинга** , выполните следующие действия.
1. Запишите имя пользователя или адрес электронной почты в поле **Добавить новые члены** .
1. Выберите уровень **разрешений** , который **можно просмотреть** или **изменить** , а затем нажмите кнопку **Добавить**.
...
Чтобы изменить уровень разрешений пользователя на панели **разрешений панели мониторинга** , сделайте следующее:
1. Используйте поле поиска или прокрутите список пользователей, чтобы найти пользователя.
1. При необходимости измените уровень **разрешений** .

### <a name="share-the-dashboard-link"></a>Поделиться ссылкой на панель мониторинга

Чтобы поделиться ссылкой на панель мониторинга, сделайте следующее:
* Щелкните раскрывающийся список **общий доступ** , а затем выберите **Копировать ссылку** или
* В окне **разрешения панели мониторинга** выберите **Копировать ссылку**. 

## <a name="enable-auto-refresh"></a>Включить автоматическое обновление 

1. Выберите **изменить** в меню Панель мониторинга, чтобы переключиться в режим редактирования.
1. Выберите **Автоматическое обновление**. 
 
    :::image type="content" source="media/adx-dashboards/auto-refresh.png" alt-text="Выбор автоматического обновления":::

1. Переключите параметр, чтобы **включить** автоматическое обновление. 
1. Выберите значения для **минимального интервала времени** и **частоту обновления по умолчанию**. 

    :::image type="content" source="media/adx-dashboards/auto-refresh-toggle.png" alt-text="Включить автоматическое обновление":::

1. Нажмите кнопку **Применить** и **Сохраните** панель мониторинга.

> [!NOTE]
> * Выберите наименьший минимальный интервал времени, чтобы уменьшить ненужную нагрузку на кластер. 
> * Средство просмотра панели мониторинга: 
>    * Может изменять минимальный интервал времени для личного использования. 
>    * Не удается выбрать значение, которое меньше **минимального интервала времени** , указанного в редакторе.
