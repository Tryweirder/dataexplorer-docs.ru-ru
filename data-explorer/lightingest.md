...
|-Limit,-l           |integer   |Если задано, ограничение приема до первых N файлов |Необязательно  |
|-Листонли,-List        |bool    |Если задано, отображаются только те элементы, которые были выбраны для приема| Необязательно  |
|-Инжесттимеаут   |integer  |Время ожидания для выполнения всех операций приема (в минутах). Значение по умолчанию — `60`| Необязательно  |
|-Форцесинк        |bool  |Если задано, инициирует синхронное получение. Значение по умолчанию — `false` |Необязательно  |
|-Датабатчсизе        |integer  |Устанавливает общий предельный размер (МБ, без сжатия) каждой операции приема |Необязательно  |
|-Филесинбатч            |integer |Задает ограничение числа файлов и BLOB-объектов для каждой операции приема |Необязательно  |
