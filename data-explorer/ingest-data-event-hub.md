...
> * Для сопоставления `csv` свойства добавляются к началу записи. А для `json` — в соответствии с именем, отображенным в раскрывающемся списке.
...
1. Чтобы увидеть содержимое сообщений, выполните следующий запрос:
    ...
    > [!NOTE]
    > * В Azure Data Explorer настроена политика агрегирования (пакетной обработки) для приема данных, предназначенных для оптимизации процесса. Эта политика настроена на прием данных с интервалом 5 минут или 500 МБ данных по умолчанию, поэтому могут возникать задержки. Просмотрите [политику пакетной обработки](kusto/management/batchingpolicy.md) для параметров статической обработки. 
