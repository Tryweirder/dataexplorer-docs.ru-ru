...
1. На экране **Обозреватель данных Azure (Kusto)** заполните форму следующими сведениями.
    ...
    | Параметр | Образец значения | Описание поля
    |---|---|---
    | Ограничить номер записи результата запроса| `300000` | Максимальное число записей, возвращаемых в результате |
    | Ограничить размер данных для результатов запроса | `4194304` | Максимальный размер данных в байтах, возвращаемый в результате |
    | Отключить усечение результирующего набора | `true` | Включение или отключение усечения результатов с помощью параметра запроса "Truncate" |
    | Дополнительные инструкции SET | `set query_datascope=hotcache` | Задает параметры запроса на время выполнения запроса. Параметры запроса управляют выполнением запроса и возвращением результатов. |
