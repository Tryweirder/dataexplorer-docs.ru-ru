...
ms.locfileid: "88875163"
---
# <a name="machine-learning-capability-in-azure-data-explorer"></a>Возможности машинного обучения в Azure обозреватель данных

Azure обозреватель данных — это платформа аналитики больших данных. Он используется для наблюдения за работоспособностью службы, QoS или неисправности устройств. Встроенные функции [обнаружения и прогнозирования аномалий](anomaly-detection.md) проверяют аномальное поведение. После обнаружения такого шаблона выполняется анализ основных причин (RCA) для устранения или устранения аномалий.

Процесс диагностики сложен и длительен и выполняется экспертами по доменам. Процесс включает в себя следующее:
* Выборка и присоединение дополнительных данных из разных источников для одного и того же времени
* Поиск изменений в распределении значений по нескольким измерениям
* Построение диаграммы дополнительных переменных
* Другие методы на основе базы знаний домена и интуиция

Так как эти сценарии диагностики распространены в Azure обозреватель данных, доступны подключаемые модули машинного обучения, позволяющие упростить фазу диагностики и сократить продолжительность RCA.

В обозреватель данных Azure есть три подключаемых модуля Машинное обучение: [`autocluster`](kusto/query/autoclusterplugin.md) , [`basket`](kusto/query/basketplugin.md) и [`diffpatterns`](kusto/query/diffpatternsplugin.md) . Все подключаемые модули реализуют алгоритмы кластеризации. В `autocluster` `basket` кластере подключаемых модулей и один набор записей, а `diffpatterns` подключаемый модуль объединяет различия между двумя наборами записей.

## <a name="clustering-a-single-record-set"></a>Кластеризация одного набора записей

Типичный сценарий включает набор данных, выбранный по указанным критериям:
* Временное окно, в котором отображается аномальное поведение
* Считывание устройств с высокой температурой
* Команды длинной длительности
* Пользователи с наибольшим расходом хотят быстро и легко находить общие шаблоны (сегменты) в данных. Шаблоны — это подмножество набора данных, записи которых имеют одни и те же значения в нескольких измерениях (столбцы категорий). 

Следующий запрос создает и отображает временные ряды исключений службы за период недели в 10-минутных ячейках:

**\[**[**Щелкните, чтобы запустить запрос**](https://dataexplorer.azure.com/clusters/help/databases/Samples?query=)**\]**
...
Число исключений службы сопоставляется с общим трафиком службы. Вы можете ясно видеть ежедневный шаблон для рабочих дней, с понедельника по пятницу. Количество исключений службы возрастает в середине дня, а счетчики удаляются по ночам. Плоские неструктурированные счетчики видны в выходных. Пики исключений можно обнаружить с помощью [обнаружения аномалий временных рядов](anomaly-detection.md#time-series-anomaly-detection) в Azure обозреватель данных.

Второй пик данных происходит во вторник вечера. Следующий запрос используется для дальнейшей диагностики и проверки того, является ли он резкой. Запрос перерисовывает диаграмму вокруг пика с более высоким разрешением в течение восьми часов в ячейки в одну минуту. Затем можно изучить его границы.
...
Вы увидите небольшой 2-минутный пик с 15:00 до 15:02. В следующем запросе подсчитайте исключения в этом 2-минутном окне:
...
Несмотря на то, что существует меньше тысяч исключений, трудно найти общие сегменты, так как в каждом столбце есть несколько значений. Вы можете использовать [`autocluster()`](kusto/query/autoclusterplugin.md) подключаемый модуль для мгновенного извлечения короткого списка общих сегментов и поиска интересных кластеров в течение двух минут, как показано в следующем запросе:
...
| Идентификатор сегмента | Количество | Процент | Регион | скалеунит | DeploymentId | ServiceHost |
|-----------|-------|------------------|--------|-----------|----------------------------------|--------------------------------------|
...
Из результатов, приведенных выше, самый главный сегмент содержит 65,74% от общего числа записей исключений и использует четыре измерения. Следующий сегмент гораздо менее распространен. Он содержит только 9,67% записей и использует три измерения. Другие сегменты еще менее распространены.

В автокластере используется собственный алгоритм для интеллектуального анализа нескольких измерений и извлечения интересных сегментов. «Интересное» означает, что каждый сегмент имеет значительный охват как набора записей, так и набора функций. Сегменты также расходятся, т. е. каждый из них отличается от других. Один или несколько из этих сегментов могут быть актуальны для процесса RCA. Чтобы избежать рассмотрения и оценки сегмента, автокластер извлекает только небольшой список сегментов.
...
| Идентификатор сегмента | Количество | Процент | Регион | скалеунит | DeploymentId | Трассировки | ServiceHost |
|-----------|-------|------------------|--------|-----------|----------------------------------|------------|--------------------------------------|
| 0 | 639 | 65.7407407407407 | еау | su7 | b5d1d4df547d4a04ac15885617edba57 |  | e7f60c5d-4944-42b3-922a-92e98a8e7dec |
...
В корзине реализован алгоритм *"Apriori"* для интеллектуального анализа набора элементов. Он извлекает все сегменты, уровень покрытия которых превышает пороговое значение (по умолчанию — 5%). Вы видите, что другие сегменты были извлечены с похожими, такими как сегменты 0, 1 или 2, 3.

Оба подключаемых модуля являются мощными и простыми в использовании. Их ограничение заключается в том, что кластеры используют один набор записей без каких-либо меток. Неясно, характеризуются ли извлеченные шаблоны выбранным набором записей, аномальными записями или глобальным набором записей.

## <a name="clustering-the-difference-between-two-records-sets"></a>Кластеризация разницы между двумя наборами записей

[`diffpatterns()`](kusto/query/diffpatternsplugin.md)Подключаемый модуль ограничивает ограничение `autocluster` и `basket` . `Diffpatterns` принимает два набора записей и извлекает основные сегменты, которые отличаются. Один набор обычно содержит нетипичный набор записей, который анализируется. Один из них анализируется `autocluster` и `basket` . Другой набор содержит набор записей ссылок, базовый план.

В приведенном ниже запросе `diffpatterns` находит интересные кластеры в течение двух минут, которые отличаются от кластеров в базовом плане. Окно "базовые показатели" определяется как восемь минут до 15:00, когда запустится пик. Расширение выполняется по двоичному столбцу (AB) и указывает, принадлежит ли определенная запись к базовому плану или к аномальному набору. `Diffpatterns` реализует контролируемый алгоритм обучения, в котором две метки классов были созданы аномальными и небазовыми флагами (AB).

**\[**[**Щелкните, чтобы запустить запрос**](https://dataexplorer.azure.com/clusters/help/databases/Samples?query=)**\]**
...
Наиболее главным сегментом является тот же сегмент, который был извлечен `autocluster` . Его покрытие в течение 2-минутного окна также составляет 65,74%. Однако его покрытие в периоде с 8-минутным базовым периодом составляет всего 1,7%. Разница составляет 64,04%. Похоже, эта разница связана с аномальными пиками. Чтобы проверить это допущение, разделите исходную диаграмму на записи, принадлежащие этому сегменту проблем, и записи из других сегментов. См. запрос ниже.
...
Подключаемые модули Машинное обучение Azure обозреватель данных полезны во многих сценариях. `autocluster`И реализуют неконтролируемый `basket` алгоритм обучения и просты в использовании. `Diffpatterns` реализует защищенный алгоритм обучения и, хотя и более сложный, он предоставляет более широкие возможности для извлечения сегментов различий для RCA.

Эти подключаемые модули используются в интерактивном режиме в нерегламентированных сценариях и в автоматизированных службах мониторинга практически в реальном времени. В обозреватель данных Azure за обнаружение аномалий временных рядов следует процесс диагностики. Процесс оптимизирован для удовлетворения необходимых стандартов производительности.
