...
Поставьте в очередь сообщение, чтобы получить данные из хранилища BLOB-объектов и получить эти данные в обозреватель данных Azure. Подключение устанавливается к конечной точке приема данных в кластере Azure обозреватель данных, а для работы с этой конечной точкой создается другой клиент. Те же рекомендации применяются, как в предыдущем разделе.
